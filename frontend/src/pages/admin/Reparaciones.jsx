...
// Muestra la hora solo si el registro realmente la tiene (evita el "12:00" cuando la BD guarda solo fecha)
const fmtFull = (d) => {
  if (!d) return "—";
  const f = new Date(d);
  if (isNaN(f)) return "—";
...
const fmtBs = (n) =>
  n != null && n !== "" ? `Bs ${Number(n).toFixed(2)}` : "—";
const seg = (n, a) =>
  [n, a]
    .filter((x) => x && x !== "null" && x !== "None" && x !== "undefined")
    .join(" ")
    .trim() || "—";
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          <button onClick={onClose} style={{ ...btnG, flex: 1 }}>
            Cancelar
          </button>
          <button
            onClick={() => {
            ...
            style={{
              ...mkBtn(colorOk, colorOk === "#f59e0b" ? "#000" : "#fff"),
              flex: 1,
            }}
          >
            {labelOk || "Confirmar"}
...
function ModalDetalle({ mant, onClose, onAction, cargando }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      api
        .get(`/mantenimientos/${mant.id}/historial`)
  ...
  const agregarRep = () => {
    ...
    if (parseInt(repSel.cantidad) > cat.stock)
      return setAlerta(
        `Stock insuficiente para "${cat.nombre}". Disponible: ${cat.stock}`,
      );
    setAlerta("");
    setRepUsados((p) => [
      ...p,
      {
        repuesto_id: cat.id,
        nombre: cat.nombre,
        cantidad: parseInt(repSel.cantidad) || 1,
  ...
  const quitarRep = (id) =>
    setRepUsados((p) => p.filter((r) => r.repuesto_id !== id));
  ...
  return (
    <Modal
      ...
      subtitle={`${mant.equipo_codigo} — ${mant.equipo_nombre}`}
      onClose={onClose}
      width="820px"
    >
      ...
      >
        ...
        {mant.nivel_riesgo_ia && (
          ...
          >
            ⚠️ IA: {mant.nivel_riesgo_ia} — {mant.probabilidad_ia}%
          </span>
        )}
      </div>
      ...
      >
        <InfoCard
          label="Equipo"
          value={`${mant.equipo_codigo} — ${mant.equipo_nombre}`}
        />
        <InfoCard label="Tipo equipo" value={mant.equipo_tipo || "—"} />
        <InfoCard label="Tipo mant." value={mant.tipo || "—"} />
        <InfoCard
          label="Técnico"
          value={mant.tecnico_nombre_completo || mant.tecnico || "—"}
        />
        <InfoCard
          label="Responsable"
          value={mant.responsable_nombre_completo || "—"}
        />
        <InfoCard label="Área" value={mant.area || "—"} />
        <InfoCard label="F. Programada" value={fmt(mant.fecha_programada)} />
        <InfoCard
          label="F. Inicio"
          value={mant.fecha_inicio ? fmtFull(mant.fecha_inicio) : "—"}
        />
        <InfoCard
          label="F. Fin"
          value={mant.fecha_fin ? fmtFull(mant.fecha_fin) : "—"}
        />
      ...
      {mant.estado === "REPROGRAMAR" && (
        ...
        >
          ...
          {mant.motivo_reprogramar && (
            ...
            >
              <strong>Motivo:</strong> {mant.motivo_reprogramar}
            </p>
          )}
          {mant.fecha_sugerida_rep && (
            <p
              style={{
                margin: 0,
                color: "var(--text-primary)",
            ...
            >
              <strong>Fecha sugerida:</strong> {fmt(mant.fecha_sugerida_rep)}
            </p>
          )}
        </div>
      )}
      ...
      {mant.estado === "PENDIENTE_CONFIRMACION" && (
        ...
        >
          ...
          >
            ⏳ Esperando confirmación del responsable
          </p>
          <p
            style={{
              color: "var(--text-secondary)",
          ...
          >
            El responsable fue notificado. Cuando confirme, el estado pasará a
      ...
      {mant.estado === "REPROGRAMAR" && (
        <div style={{ marginBottom: "1rem" }}>
          ...
          <div style={{ display: "flex", gap: "10px", alignItems: "flex-end" }}>
            ...
            <button
              onClick={() => {
                if (!fechaN) return setAlerta("Selecciona una fecha");
                onAction("REPROGRAMAR_ADMIN", { fecha_programada: fechaN });
              }}
              disabled={!fechaN || cargando}
              style={{ ...btnP, opacity: !fechaN || cargando ? 0.6 : 1 }}
            >
              📅 Reprogramar
            </button>
          </div>
      ...
      {/* Iniciar */}
      {["PROGRAMADO", "PENDIENTE", "PENDIENTE_CONFIRMACION"].includes(
        mant.estado,
      ) && (
        <div style={{ marginBottom: "1rem" }}>
          <button
            ...
            disabled={cargando}
            style={{ ...btnP, opacity: cargando ? 0.6 : 1 }}
          >
            {mant.estado === "PENDIENTE_CONFIRMACION"
              ? "Confirmar y empezar mantenimiento"
        ...
        >
          ...
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            {mant.estado === "EN_PROCESO" && (
              <button
                onClick={() => onAction("PAUSAR", { observacion: obs })}
                disabled={cargando}
                style={{ ...btnWarn, opacity: cargando ? 0.6 : 1 }}
              >
                ⏸ Pausar
              </button>
            )}
          ...
          >
            ...
            >
              <div style={{ flex: 2, minWidth: "180px" }}>
                ...
                <select
                  style={S.inp}
                  value={repSel.repuesto_id}
                  onChange={(e) =>
                    setRepSel((r) => ({ ...r, repuesto_id: e.target.value }))
                  }
                >
                ...
                >
                  <thead>
                    <tr style={{ borderBottom: "1px solid var(--border)" }}>
                      {["Repuesto", "Cant.", "P.Unit.", "Subtotal", ""].map(
                        (h) => (
                          <th
                            key={h}
                            style={{
                    ...
                    {repUsados.map((r) => (
                      ...
                      >
                        ...
                        <td style={{ padding: "8px 10px" }}>
                          <button
                            onClick={() => quitarRep(r.repuesto_id)}
                            style={{
                              background: "rgba(239,68,68,0.12)",
                              border: "1px solid #ef4444",
                              color: "#ef4444",
          ...
          >
            ...
            >
              <div>
                <label style={S.lbl}>
                  Descripción de la solución / trabajo realizado *
                </label>
                <textarea
                  rows={4}
                  ...
                  placeholder="Describe detalladamente el trabajo realizado, piezas reemplazadas, configuraciones realizadas..."
                />
              </div>
              <div
              ...
              >
                <div style={{ flex: 1 }}>
                  <label style={S.lbl}>
                    Costo total del mantenimiento (Bs)
                  </label>
                  <input
                    type="number"
                    min={0}
                ...
                <button
                  onClick={() => {
                    if (!solucion.trim())
                      return setAlerta(
                        "Describe la solución aplicada antes de finalizar",
                      );
                    onAction("FINALIZAR", {
              ...
              {repUsados.length > 0 && (
                ...
                >
                  ℹ️ Se descontarán {repUsados.length} repuesto(s) del
      ...
      {mant.repuestos?.length > 0 && (
        <div style={{ marginBottom: "1rem" }}>
          ...
          >
            Repuestos registrados en este mantenimiento
          </p>
          <div
            style={{
              border: "1px solid var(--border)",
              ...
              <tbody>
                ...
                >
                  ...
                  >
                    ...
                    {mant.repuestos
                      .reduce(
                        (a, r) => a + r.cantidad * (r.precio_unitario || 0),
                        0,
                      )
                      .toFixed(2)}
                  </td>
...
function ModalNuevo({
  ...
  const [form, setForm] = useState({
    ...
    tecnico:
      rol === "EMPLEADO" ? seg(currentUser?.nombre, currentUser?.apellido) : "",
  ...
  const agregarRep = () => {
    if (!repSel.repuesto_id) return setAlerta("Selecciona un repuesto");
    const cat = repuestosCat.find(
      (r) => String(r.id) === String(repSel.repuesto_id),
    );
    if (!cat) return;
    if (form.repuestos.find((r) => String(r.repuesto_id) === String(cat.id)))
      return setAlerta("Este repuesto ya fue agregado");
    if (parseInt(repSel.cantidad) > cat.stock)
      return setAlerta(
        `Stock insuficiente para "${cat.nombre}". Disponible: ${cat.stock}`,
  ...
  const quitarRep = (id) =>
    setForm((f) => ({
      ...f,
      repuestos: f.repuestos.filter((r) => r.repuesto_id !== id),
    }));

  const guardar = async () => {
    if (!form.equipo_id) return setAlerta("Selecciona un equipo");
    if (!form.descripcion_problema)
      return setAlerta("La descripción del problema es obligatoria");
    setBusy(true);
    setAlerta("");
    try {
  ...
  const totalRep = form.repuestos.reduce(
    (a, r) => a + r.cantidad * r.precio,
    0,
  );
  ...
  return (
    <Modal
      ...
      subtitle={
        sol
          ? `Solicitud #${sol.id} — ${sol.equipo_codigo}`
    ...
    >
      ...
      {sol?.usuario_nombre && (
        ...
        >
          <InfoCard
            label="Personal"
            value={seg(sol.usuario_nombre, sol.usuario_apellido)}
          />
          ...
          <InfoCard
            label="Equipo"
            value={`${sol.equipo_codigo} — ${sol.equipo_nombre}`}
          />
      ...
      >
            ...
            >
              ...
              {!sol && (
                <button
                  ...
                  style={{ ...btnDng, padding: "4px 10px", fontSize: "12px" }}
                >
                  Cambiar
                </button>
              )}
                          ...
                          background:
                            e.estado === "DISPONIBLE"
                              ...
                              : e.estado === "ASIGNADO"
                                ...
                                : "rgba(239,68,68,0.12)",
                          border: `1px solid ${e.estado === "DISPONIBLE" ? "#22c55e" : e.estado === "ASIGNADO" ? "#f59e0b" : "#ef4444"}`,
                          borderRadius: "6px",
                          padding: "2px 8px",
                        }}
                      >
        ...
        <div>
          ...
          {rol === "EMPLEADO" ? (
            ...
            <select
              ...
              onChange={(e) => {
                const t = tecnicos.find((x) => String(x.id) === e.target.value);
                setForm((f) => ({
                  ...f,
                  tecnico_usuario_id: e.target.value,
                  tecnico: t ? seg(t.nombre, t.apellido) : "",
                }));
              }}
            >
        ...
        <div>
          ...
          />
          {form.estado === "PENDIENTE_CONFIRMACION" &&
            form.fecha_programada && (
              ...
              >
                ℹ️ El responsable será notificado para confirmar disponibilidad
              </p>
            )}
        </div>
        ...
        <div style={{ gridColumn: "1/-1" }}>
          ...
          <textarea
            ...
            placeholder="Describe el problema detectado, síntomas, error reportado..."
          />
        </div>
        ...
        <div style={{ gridColumn: "1/-1" }}>
          <label style={S.lbl}>
            🔧 Solución / trabajo realizado (opcional)
          </label>
          <textarea
            rows={3}
            style={{ ...S.inp, resize: "vertical" }}
            ...
            placeholder="Si ya se realizó el trabajo, descríbelo aquí..."
          />
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          {form.repuestos.length > 0 ? `(${form.repuestos.length})` : ""}
        </p>
        <div
          style={{
            display: "flex",
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "flex-end" }}>
            <button onClick={agregarRep} style={btnP}>
              + Agregar
            </button>
          </div>
        </div>
      ...
      <div style={{ display: "flex", gap: "10px", justifyContent: "flex-end" }}>
        ...
        <button
          onClick={guardar}
          disabled={busy}
          style={{ ...btnP, opacity: busy ? 0.6 : 1, padding: "10px 24px" }}
        >
...
export default function Reparaciones() {
  ...
  const [reasigForm, setReasigForm] = useState({
    tecnico: "",
    tecnico_usuario_id: "",
  });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      let tecs = [];
      try {
        const r = await api.get("/empleados/tecnicos");
        tecs = r.data || [];
      } catch {}
  ...
  const loadReportes = async () => {
    try {
      ...
      showMsg("error", "Error cargando reportes");
    }
  };
  useEffect(() => {
  ...
  const accionMant = async (accion, extra = {}) => {
    ...
    try {
      ...
      const map = {
        ...
        REPROGRAMAR_ADMIN: () =>
          api.put(`/mantenimientos/${id}/reprogramar-admin`, extra),
      };
  ...
  const resumen = {
    pend_conf: mantenimientos.filter(
      (m) => m.estado === "PENDIENTE_CONFIRMACION",
    ).length,
    ...
    reprogramar: mantenimientos.filter((m) => m.estado === "REPROGRAMAR")
      .length,
  ...
  const TABS = [
    {
      ...
      badge: resumen.en_proceso + resumen.pausados + resumen.reprogramar,
    },
    { key: "solicitudes", label: "Solicitudes", badge: solPend },
    { key: "reportes", label: "Reportes", badge: 0 },
  ];
  const tabsVis =
    rol === "EMPLEADO" ? TABS.filter((t) => t.key === "mantenimientos") : TABS;
    ...
    <AdminLayout>
      ...
      >
        <div>
          ...
          >
            Gestión del ciclo de mantenimiento de equipos
            {rol === "EMPLEADO" && (
              <span
                style={{
                  marginLeft: "10px",
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          ...
          <button style={btnP} onClick={() => setModalNuevo("nuevo")}>
            + Nuevo mantenimiento
          </button>
        </div>
      </div>
      ...
      >
        <Stat
          label="En curso"
          value={resumen.en_proceso + resumen.pausados}
          color="var(--accent)"
          hint={
            resumen.pausados
              ? `${resumen.pausados} en pausa`
              : "trabajos activos"
          }
        />
        <Stat
          label="Programados"
          value={resumen.programados + resumen.pend_conf}
          color="#3b82f6"
          hint={
            resumen.pend_conf
              ? `${resumen.pend_conf} sin confirmar`
              : "agenda futura"
          }
        />
        <Stat
          label="Por atender"
          value={solPend + resumen.reprogramar}
          color="#f59e0b"
          hint={`${solPend} solicitud(es) · ${resumen.reprogramar} reprog.`}
        />
        <Stat
          label="Finalizados"
        ...
        <div>
              ...
              {reprogr.map((m) => (
                ...
                >
                  <div style={{ flex: 1, minWidth: "200px" }}>
                    ...
                    >
                      Reprogramación solicitada — {m.equipo_codigo}{" "}
                      {m.equipo_nombre}
                    </p>
                    <p
                      style={{
                    ...
                    >
                      {m.motivo_reprogramar
                        ? `Motivo: ${m.motivo_reprogramar}`
                        : "Sin motivo indicado"}
                      {m.fecha_sugerida_rep
                        ? ` · Fecha sugerida: ${fmt(m.fecha_sugerida_rep)}`
                        : ""}
                    </p>
                  </div>
                  <button
                    onClick={async () => {
                      try {
                        const r = await api.get(`/mantenimientos/${m.id}`);
                        setModalDetalle({ ...m, ...r.data });
                      } catch {
                        setModalDetalle(m);
                      }
                    }}
                    style={{ ...btnP, padding: "7px 16px", fontSize: "12px" }}
                  >
                    Reprogramar
                  </button>
                </div>
          ...
          >
            <input
              ...
              placeholder="Buscar equipo, técnico o responsable..."
              value={buscarMant}
              ...
              {EFILTRO.map((f) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: "8px",
                    border:
                      filtroEstado === f.k ? "none" : "1px solid var(--border)",
                    background:
                      filtroEstado === f.k
                        ? "var(--accent)"
                        : "var(--bg-surface)",
                    color:
                      filtroEstado === f.k ? "#fff" : "var(--text-secondary)",
                    fontWeight: 600,
                    fontSize: "12px",
                    cursor: "pointer",
                    fontFamily: "inherit",
            ...
            >
              {mantFiltrados.length} registros
            </span>
          </div>
                      ...
                      <tr>
                        ...
                        >
                          ...
                          <p style={{ margin: "4px 0 0", fontSize: "12px" }}>
                            Registra el primero con el botón "Nuevo
                            mantenimiento"
                          </p>
                        </td>
                      </tr>
                      ...
                      mantFiltrados.map((m) => (
                        <tr
                          ...
                          onMouseEnter={(e) =>
                            (e.currentTarget.style.background =
                              "var(--hover,rgba(108,99,255,0.03))")
                          }
                          onMouseLeave={(e) =>
                        ...
                        >
                          ...
                          >
                            {m.tecnico_nombre_completo || m.tecnico || "—"}
                            {rol === "EMPLEADO" &&
                              m.tecnico_usuario_id === currentUser.id && (
                                <span
                                  style={{
                                    display: "block",
                              ...
                              >
                                <span
                                  style={{
                                    ...
                                    borderRadius: "50%",
                                    background:
                                      m.nivel_riesgo_ia === "CRITICO"
                                        ? "#ef4444"
                                        : m.nivel_riesgo_ia === "ALTO"
                                          ? "#f59e0b"
                                          : "#eab308",
                                  }}
                                />
          ...
          {reprogr.length > 0 && (
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                >
                  {reprogr.length} mantenimiento(s) requieren reprogramación
                </p>
                <p
                  style={{
                    color: "var(--text-secondary)",
                ...
                >
                  Ve al tab Mantenimientos para fijar la nueva fecha.
                </p>
              </div>
              <button
                onClick={() => setTab("mantenimientos")}
                style={{ ...btnDng, padding: "6px 14px", fontSize: "12px" }}
              >
                Ver →
              </button>
            </div>
                    ...
                    solicitudes.map((s) => (
                      ...
                      >
                        ...
                        <td style={{ padding: "12px 14px" }}>
                          ...
                          >
                            {seg(s.usuario_nombre, s.usuario_apellido)}
                          </p>
                          <p
                            style={{
                              margin: 0,
                                ...
                                s.estado === "PENDIENTE"
                                  ...
                                  : s.estado === "EN_PROCESO"
                                    ...
                                    : s.estado === "FINALIZADO"
                                      ? "#22c55e"
                                      : "var(--text-muted)",
                              border: `1px solid ${s.estado === "PENDIENTE" ? "#f59e0b" : s.estado === "EN_PROCESO" ? "var(--accent)" : s.estado === "FINALIZADO" ? "#22c55e" : "var(--border)"}`,
                              borderRadius: "6px",
                              padding: "2px 10px",
                              fontSize: "11px",
                              fontWeight: 700,
      ...
      {tab === "reportes" && (
        <div>
          ...
          >
            ...
            >
              <div>
                <label style={S.lbl}>Equipo</label>
                <input
                  style={S.inp}
                  placeholder="Código o nombre"
                  value={filtros.equipo}
                  onChange={(e) =>
                    setFiltros((f) => ({ ...f, equipo: e.target.value }))
                  }
                />
              </div>
              <div>
                <label style={S.lbl}>Personal</label>
                <input
                  style={S.inp}
                  placeholder="Nombre"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFiltros((f) => ({ ...f, fecha_ini: e.target.value }))
                  }
                />
              </div>
              <div>
            ...
            <div style={{ display: "flex", gap: "10px", marginTop: "1rem" }}>
              <button style={btnP} onClick={loadReportes}>
                🔍 Buscar
              </button>
              <button
                style={btnG}
          ...
          {reportes.length > 0 && (
            ...
            >
              <Stat
                label="Registros"
                value={totales.registros || 0}
                color="var(--accent)"
              />
              <Stat
                label="Repuestos (u)"
                value={totales.repuestos_qty || 0}
                color="#3b82f6"
              />
              <Stat
                label="Costo repuestos"
                value={`Bs ${Number(totales.repuestos_costo || 0).toFixed(2)}`}
                color="#f59e0b"
              />
              <Stat
                label="Costo mant."
                value={`Bs ${Number(totales.costo_total || 0).toFixed(2)}`}
                color="#22c55e"
              />
              <Stat
                label="Total general"
                value={`Bs ${Number(totales.costo_general || 0).toFixed(2)}`}
                color="#ef4444"
              />
            </div>
          )}
      ...
      {modalNuevo && (
        <ModalNuevo
          ...
          onGuardado={() => {
            load();
            showMsg("success", "✅ Mantenimiento registrado correctamente");
          }}
        />
      )}
      ...
      {modalReasig && (
        <Overlay onClose={() => setModalReasig(null)}>
          ...
          >
            ...
            >
              Mantenimiento #{modalReasig.id} — Actual:{" "}
              <strong>{reasigForm.tecnico || "Sin asignar"}</strong>
            </p>
            <label style={S.lbl}>Nuevo técnico</label>
            <select
              style={{ ...S.inp, marginBottom: "1.25rem" }}
              value={reasigForm.tecnico_usuario_id}
              onChange={(e) => {
                const t = tecnicos.find((x) => String(x.id) === e.target.value);
                setReasigForm({
                  tecnico_usuario_id: e.target.value,
            ...
            >
              <button style={btnG} onClick={() => setModalReasig(null)}>
                Cancelar
              </button>
              <button
                style={{ ...mkBtn("#f59e0b", "#000") }}
                onClick={reasignar}
              >
                🔄 Reasignar
              </button>
