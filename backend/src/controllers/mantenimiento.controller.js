  ...
  try {
    await db.query(
      `INSERT INTO mantenimiento_historial
         (mantenimiento_id, usuario_id, accion, estado_anterior, estado_nuevo, observacion, fecha)
       VALUES (?,?,?,?,?,?,?)`,
      [mant_id, usuario_id || null, accion, ant || null, nuevo || null, obs || null, ahoraMySQL()]
    );
  ...
  try {
    ...
    await db.query(
      `INSERT INTO inventario_movimientos
         (repuesto_id, tipo, cantidad, stock_antes, stock_despues, motivo, referencia, usuario_id, fecha)
       VALUES (?,?,?,?,?,?,?,?,?)`,
      [repuesto_id, 'SALIDA', cantidad, sa, sd, `Usado en ${referencia}`, referencia, usuario_id || null, ahoraMySQL()]
    );
    return { ok: true };
...
exports.getMisSolicitudes = async (req, res) => {
  try {
    const [rows] = await db.query(`
      SELECT sm.id, sm.descripcion, sm.prioridad, sm.estado,
             sm.fecha_solicitud, sm.equipo_id,
             e.codigo AS equipo_codigo, e.nombre AS equipo_nombre, t.nombre AS equipo_tipo
      FROM solicitudes_mantenimiento sm
      JOIN equipos e ON e.id = sm.equipo_id
      LEFT JOIN tipos_equipo t ON t.id = e.tipo_id
...
exports.createSolicitud = async (req, res) => {
  try {
    ...
    const [[sol]] = await db.query(
      `SELECT id FROM solicitudes_mantenimiento WHERE equipo_id=? AND estado IN ('PENDIENTE','EN_PROCESO') LIMIT 1`,
      [equipo_id]
    );
    ...
    const [[mant]] = await db.query(
      `SELECT id FROM mantenimientos WHERE equipo_id=?
       AND estado IN ('PENDIENTE_CONFIRMACION','PROGRAMADO','EN_PROCESO','PAUSADO','REPROGRAMAR') LIMIT 1`,
      [equipo_id]
    );
    if (mant) return res.status(400).json({ message: 'El equipo ya tiene un mantenimiento activo' });

    await db.query(
      `INSERT INTO solicitudes_mantenimiento (usuario_id,equipo_id,descripcion,prioridad,estado,fecha_solicitud)
       VALUES (?,?,?,?,'PENDIENTE',?)`,
      [req.user.id, equipo_id, descripcion, prioridad || 'MEDIA', ahoraMySQL()]
    );
...
exports.reenviarSolicitud = async (req, res) => {
  try {
    ...
    await pushMany(admins, {
      ...
      cuerpo: `Solicitud reenviada para ${eq?.codigo} — ${eq?.nombre}.`,
      icono: '/icon-192.png', url: '/admin/reparaciones', tag: `sol-reenvio-${id}`,
    });
...
exports.getMantenimientos = async (req, res) => {
  try {
    const [rows] = await db.query(`
      SELECT
        m.id, m.equipo_id, m.solicitud_id,
...
exports.createMantenimiento = async (req, res) => {
  try {
    ...
    // Estado: si viene de solicitud → siempre EN_PROCESO
    let estadoFinal;
    if (solicitud_id)            estadoFinal = 'EN_PROCESO';
    else if (estado && estado !== '') estadoFinal = estado;
    else                         estadoFinal = 'EN_PROCESO';
    ...
    const [result] = await db.query(`
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
    `, [
      equipo_id, solicitud_id || null,
      tipo || 'CORRECTIVO', descripcion,
      descripcion_problema || null, descripcion_solucion || null,
      tecnico || null, tecnico_usuario_id || null,
      ahora, fecha_programada || null,
      costo || null, estadoFinal,
    ]);
    ...
    if (Array.isArray(repuestos) && repuestos.length > 0) {
      for (const r of repuestos) {
        ...
        await db.query(
          `INSERT INTO mantenimiento_repuestos (mantenimiento_id,repuesto_id,cantidad,precio_unitario) VALUES (?,?,?,?)`,
    ...
    // Notificar si PENDIENTE_CONFIRMACION
    if (estadoFinal === 'PENDIENTE_CONFIRMACION' && fecha_programada) {
      ...
      if (resp) {
        ...
        await push(resp.id, {
          ...
          cuerpo: `Tu equipo ${eq?.codigo} tiene mantenimiento el ${fecha_programada}. Confirma disponibilidad.`,
          icono:  '/icon-192.png', url: '/personal/mi-equipo', tag: `prog-${mantId}`,
        });
      }
    }
...
exports.confirmarDisponibilidad = async (req, res) => {
  try {
    ...
    await db.query(
      `UPDATE mantenimientos SET estado='PROGRAMADO', confirmado_por=?, fecha_confirmacion=? WHERE id=?`,
      [req.user.id, ahoraMySQL(), id]
    );
    ...
    if (m.tecnico_usuario_id) {
      await notificar(m.tecnico_usuario_id, 'MANT_CONFIRMADO',
        `✅ Confirmado — ${m.codigo}`,
        `Responsable confirmó disponibilidad para ${m.codigo} el ${m.fecha_programada || '—'}.`,
        { mantenimiento_id: parseInt(id) }
      );
      await push(m.tecnico_usuario_id, {
        titulo: `✅ Mantenimiento confirmado — ${m.codigo}`,
        cuerpo: `El responsable confirmó. Fecha: ${m.fecha_programada || '—'}.`,
        icono:  '/icon-192.png', url: '/admin/reparaciones', tag: `confirm-${id}`,
      });
    }
...
exports.solicitarReprogramacion = async (req, res) => {
  try {
    ...
    const admins = await getAdmins();
    for (const uid of admins) {
      await notificar(uid, 'REPROGRAMAR',
        `📅 Reprogramación — ${m.codigo}`,
        `Solicitud de reprogramar ${m.codigo}. Motivo: ${motivo || '—'}.`,
        { mantenimiento_id: parseInt(id) }
      );
    }
    await pushMany(admins, {
      titulo: `📅 Reprogramación solicitada — ${m.codigo}`,
      cuerpo: motivo || 'El responsable solicita cambiar la fecha.',
      icono:  '/icon-192.png', url: '/admin/reparaciones', tag: `reprg-${id}`,
    });
    res.json({ message: 'Solicitud de reprogramación enviada' });
  } catch (err) { res.status(500).json({ message: err.message }); }
};
...
exports.reprogramarAdmin = async (req, res) => {
  try {
    ...
    await registrarHistorial(id, req.user?.id, 'REPROGRAMAR', m.estado, 'PENDIENTE_CONFIRMACION', `Nueva fecha: ${fecha_programada}`);
    ...
    if (resp) {
      ...
      await push(resp.id, {
        ...
        cuerpo: `Nueva fecha: ${fecha_programada}. Por favor confirma tu disponibilidad.`,
        icono:  '/icon-192.png', url: '/personal/mi-equipo', tag: `rep-${id}`,
      });
    }
...
exports.iniciarMantenimiento = async (req, res) => {
  try {
    ...
    if (!m) return res.status(404).json({ message: 'No encontrado' });
    if (!['PROGRAMADO','PAUSADO','PENDIENTE','PENDIENTE_CONFIRMACION'].includes(m.estado))
      return res.status(400).json({ message: `No se puede iniciar desde ${m.estado}` });

    const ahora = ahoraMySQL();
    await db.query("UPDATE mantenimientos SET estado='EN_PROCESO', fecha_inicio=? WHERE id=?", [ahora, id]);
    ...
    if (resp) {
      await notificar(resp.id, 'MANT_INICIADO', `⚙️ Iniciado — ${m.codigo}`,
        `El técnico inició el mantenimiento de tu equipo ${m.codigo}.`,
        { mantenimiento_id: parseInt(id), equipo_id: m.equipo_id }
      );
      await push(resp.id, {
        titulo: `⚙️ Mantenimiento iniciado — ${m.codigo}`,
        cuerpo: 'El técnico ya está trabajando en tu equipo.',
        icono:  '/icon-192.png', url: '/personal/mi-equipo', tag: `inicio-${id}`,
      });
    }
...
exports.finalizarMantenimiento = async (req, res) => {
  try {
    ...
    if (Array.isArray(repuestos) && repuestos.length > 0) {
      ...
      for (const r of repuestos) {
        ...
        if (exist) {
          ...
          await db.query(
            'INSERT INTO mantenimiento_repuestos (mantenimiento_id,repuesto_id,cantidad,precio_unitario) VALUES (?,?,?,?)',
            [id, r.repuesto_id, r.cantidad, precio]
          );
        }
    ...
    const ahora = ahoraMySQL();
    await db.query(
      `UPDATE mantenimientos SET estado='FINALIZADO', fecha_fin=?, descripcion_solucion=?, costo=? WHERE id=?`,
      [ahora, descripcion_solucion || null, costo || m.costo || 0, id]
    );
    ...
    if (resp) {
      ...
      await push(resp.id, {
        titulo: `✅ Mantenimiento finalizado — ${m.codigo}`,
        cuerpo: `Tu equipo ${m.codigo} ya está disponible. ${descripcion_solucion ? descripcion_solucion.substring(0,80) : ''}`,
        icono:  '/icon-192.png', url: '/personal/mi-equipo', tag: `fin-${id}`,
      });
    }
...
exports.getRepuestos = async (req, res) => {
  try {
    const [rows] = await db.query(
      'SELECT id, nombre, tipo, stock, precio FROM repuestos WHERE stock > 0 ORDER BY nombre'
    );
    res.json(rows);
  } catch (err) { res.status(500).json({ message: err.message }); }
};
...
exports.getReportes = async (req, res) => {
  try {
    ...
    if (fecha_ini) { where += ' AND DATE(m.fecha_inicio)>=?'; params.push(fecha_ini); }
    if (fecha_fin) { where += ' AND DATE(m.fecha_inicio)<=?'; params.push(fecha_fin); }

    const [rows] = await db.query(`
      SELECT m.id, m.tipo, m.descripcion_problema, m.descripcion_solucion,
             m.tecnico, m.fecha_inicio, m.fecha_fin, m.fecha_programada,
             m.costo, m.estado,
             e.codigo AS equipo_codigo, e.nombre AS equipo_nombre, tp.nombre AS equipo_tipo,
             u_tec.nombre AS tecnico_nombre_u, u_tec.apellido AS tecnico_apellido_u,
             u_resp.nombre AS usuario_nombre, u_resp.apellido AS usuario_apellido,
             ar.nombre AS area,
    ...
    const data = rows.map(r => ({
      ...r,
      tecnico: seg(r.tecnico_nombre_u, r.tecnico_apellido_u) || r.tecnico || '—',
    }));
...
exports.getMisMantenimientos = async (req, res) => {
  try {
    const [rows] = await db.query(`
      SELECT
        m.id, m.tipo, m.estado, m.descripcion,
        m.descripcion_problema, m.descripcion_solucion, m.costo,
        m.fecha_inicio, m.fecha_fin, m.fecha_programada,
        m.motivo_reprogramar, m.fecha_sugerida_rep,
        e.id    AS equipo_id,
...
exports.getNotificacionesUsuario = async (req, res) => {
  try {
    ...
    res.json(rows.map(r => ({
      ...r,
      datos_json: (() => { try { return JSON.parse(r.datos_json); } catch { return {}; } })(),
    })));
  } catch (err) { res.status(500).json({ message: err.message }); }
};
